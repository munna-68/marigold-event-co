...
export default function Browse() {
  ...
  return <div className="app-shell page-with-rail"><SiteHeader />
    <main className="catalog-page">
      <div className="catalog-intro"><div><p className="eyebrow eyebrow--gold" data-reveal="up">THE RENTAL FIELD GUIDE</p><h1 data-reveal="up" style={delay(90)}>Pieces worth<br /><em>planning around.</em></h1></div><div className="catalog-intro__note" data-reveal="up" style={delay(180)}><div className="route-milestone route-milestone--ink"><span>01</span><i /><p>Find the piece</p></div><p>Start wide or get specific. Each item shows its rental rate, refundable deposit, and its own availability before it joins your quote.</p></div></div>
      <div className="catalog-layout">
        <aside className="catalog-rail" data-reveal="up">
        ...
        <section className="catalog-results">
          <div className="results-toolbar" data-reveal="up"><p><b className="count-up" data-count-to={visible.length}>{visible.length}</b> pieces in view</p><label className="search-box"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search inventory" aria-label="Search inventory" /></label></div>
          <div className="inventory-grid">{visible.map((item, index) => <Link href={`/rentals/${item.id}`} className="rental-card" key={item.id} data-reveal="up" style={delay((index % 3) * 80)}><div className="rental-card__image"><img src={item.image} alt="" /><span>{item.category}</span></div><div className="rental-card__body"><div><h2>{item.name}</h2><p>{item.shortDescription}</p></div><div className="rental-card__meta"><span><b>{formatMoney(item.price)}</b> rental</span><span><b>{formatMoney(item.deposit)}</b> deposit</span></div><div className="card-availability"><i /> Item calendar ready</div><div className="card-arrow">Choose a date <ArrowRight size={16} /></div></div></Link>)}</div>
